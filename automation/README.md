...
Supported runner values are `auto`, `codex`, `claude`, and `hermes`. Codex runs with `--ask-for-approval never`, `workspace-write` sandboxing, and the prompt on stdin. Claude Code runs with `--print`, `--input-format text`, `--no-session-persistence`, `--permission-mode bypassPermissions`, `--add-dir <repo_root>`, and the prompt on stdin. Hermes runs one-shot with `--safe-mode` and `--in <repo_root>`, so a run carries no operator config, memory, plugins or MCP servers, and takes its provider and model from `HERMES_INFERENCE_PROVIDER` and `HERMES_INFERENCE_MODEL`. Safe mode does not narrow the toolset, so the wrapper pins it to `terminal,file,code_execution,todo` — the default set also hands the model `delegate_task`, memory and the skills tools, which let a run spawn a second agent, keep state for the next one, or load a skill nobody asked for. Each invocation gets a throwaway `HERMES_HOME`, because sessions and memories are per-home and a shared one would let a later run read what an earlier one saw. Set `REPO_AUTOMATION_HERMES_USAGE_DIR` to collect a `*.usage.json` (which model and provider actually served the turn, and how many calls it took) and a `*.controls.json` (what that session was allowed to do) per agent session. Override executables with `REPO_AUTOMATION_CODEX_BIN`, `REPO_AUTOMATION_CLAUDE_BIN` or `REPO_AUTOMATION_HERMES_BIN`; legacy `OWLORY_CODEX_BIN` is still accepted for Codex. Override the Claude permission mode with `REPO_AUTOMATION_CLAUDE_PERMISSION_MODE` when a consumer repository needs a stricter local policy.
