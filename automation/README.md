...
Supported runner values are `auto`, `codex`, `claude`, and `hermes`. Codex runs with `--ask-for-approval never`, `workspace-write` sandboxing, and the prompt on stdin. Claude Code runs with `--print`, `--input-format text`, `--no-session-persistence`, `--permission-mode bypassPermissions`, `--add-dir <repo_root>`, and the prompt on stdin. Hermes runs one-shot with `--ignore-rules` and `--in <repo_root>` in a throwaway `HERMES_HOME`, seeded with `automation/supervisor/hermes-benchmark.yaml`, and takes its provider and model from `HERMES_INFERENCE_PROVIDER` and `HERMES_INFERENCE_MODEL`. The wrapper sets `HERMES_SAFE_MODE=1` in the environment but does **not** pass `--safe-mode`: the flag also sets `HERMES_IGNORE_USER_CONFIG`, which would discard that config profile and fall back to Hermes' built-in defaults. So plugins, MCP servers, webhooks and shell hooks are off, `AGENTS.md`/memory injection is off, and the deployed operating posture is kept. The wrapper also exports `TERMINAL_CWD=<repo_root>`, because `--in` only moves the process: the terminal, file and code_execution tools resolve their own working directory and prefer `TERMINAL_CWD`, so without it a session edits files outside the checkout it was handed. The toolset is pinned to `terminal,file,code_execution,todo`; the default set also hands the model `delegate_task`, `memory` and `session_search`. The throwaway home carries no `.env`, so credentials must come from the environment. Set `REPO_AUTOMATION_HERMES_USAGE_DIR` to collect a `*.usage.json` (which model and provider actually served the turn, and how many calls it took) and a `*.controls.json` (revision, config profile and hash, toolset) per agent session. Override executables with `REPO_AUTOMATION_CODEX_BIN`, `REPO_AUTOMATION_CLAUDE_BIN` or `REPO_AUTOMATION_HERMES_BIN`; legacy `OWLORY_CODEX_BIN` is still accepted for Codex. Override the Claude permission mode with `REPO_AUTOMATION_CLAUDE_PERMISSION_MODE` when a consumer repository needs a stricter local policy.
