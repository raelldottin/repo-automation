...
Codex runs with the existing no-approval, workspace-write invocation. Claude Code runs non-interactively with prompt stdin, `--print`, `--input-format text`, `--no-session-persistence`, `--permission-mode bypassPermissions`, and `--add-dir <repo_root>`. Hermes runs one-shot with `--ignore-rules` and `--in <repo_root>` in a fresh `HERMES_HOME` seeded with `automation/supervisor/hermes-benchmark.yaml`; it takes its provider and model from `HERMES_INFERENCE_PROVIDER` and `HERMES_INFERENCE_MODEL`. The wrapper sets `HERMES_SAFE_MODE=1` in the environment rather than passing `--safe-mode`, because the flag also sets `HERMES_IGNORE_USER_CONFIG` and would throw that config profile away. Plugins, MCP servers, webhooks, shell hooks, `AGENTS.md`/memory injection and preloaded skills are all off; the deployed posture is kept. It also exports `TERMINAL_CWD=<repo_root>`: `--in` moves the process, but the terminal, file and code_execution tools prefer `TERMINAL_CWD` when they resolve their own working directory, and an unset one puts the session's edits outside the checkout it was given. The toolset is pinned to `terminal,file,code_execution,todo` because the default set includes `delegate_task`, `memory` and `session_search`, and a run that delegates or remembers is no longer the run that was dispatched. The fresh home has no `.env`, so credentials come from the environment. `REPO_AUTOMATION_HERMES_USAGE_DIR` collects a per-session usage report and the controls it ran under.
